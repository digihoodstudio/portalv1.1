...
export default function AssistantPanel() {
  ...
  const resetChat = () => { vapiRef.current?.stop(); setMessages([INITIAL]); setCallStatus("idle"); setVapiError(""); };
  ...
  return (
    <section id="assistant" className="scroll-mt-28 rounded-[24px] border border-white/[0.06] bg-surface overflow-hidden">
      <div className="flex flex-col gap-2 border-b border-white/10 p-6 md:p-8">
        ...
        <p className="text-sm text-foreground/50">Call Maya — our voice AI agent — to experience a live demo conversation.</p>
      </div>

      <div className="flex flex-col items-center justify-center p-6 md:p-10 dark:bg-[#1E1E1E]/40 bg-surface-elevated min-h-[480px] md:min-h-[560px]">
        {/* Phone frame */}
        ...
        {/* Voice features */}
        <div className="mt-6 grid grid-cols-2 gap-3 max-w-[320px] w-full">
          {voiceFeatures.map((f) => (
            <div key={f.text} className="flex items-center gap-2 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2.5">
              <f.icon size={13} className="text-gold flex-shrink-0" />
              <span className="text-[10px] text-foreground/60 leading-tight">{f.text}</span>
            </div>
          ))}
        </div>
